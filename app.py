...
def build_scenario_gallery_cards() -> List[dict[str, Any]]:
    ...
    return scenario_gallery_module.build_scenario_gallery_cards()
...
def build_scenario_gallery_manifest_previews(
    *, n_runs: int = 100, n_years: int = 15, seed: int = 42
    ...
    return scenario_gallery_module.build_scenario_gallery_manifest_previews(
        n_runs=n_runs,
        n_years=n_years,
        seed=seed,
    )
...
def build_scenario_gallery_guided_apply_plan(
    *, n_runs: int = 100, n_years: int = 15, seed: int = 42
    ...
    return scenario_gallery_module.build_scenario_gallery_guided_apply_plan(
        n_runs=n_runs,
        n_years=n_years,
        seed=seed,
    )
...
def render_result_causal_overview(agg: pd.DataFrame, params: dict):
    ...
    with st.expander(audit_label, expanded=view.get("deeper_review_default_expanded", False)):
        ...
        if packet.get("adaptation_signal_trace"):
            st.markdown("**Anpassungs- und Drucksignale**")
            st.dataframe(
        ...
        if packet.get("counterintuitive_findings"):
            st.markdown("**Gegenintuition prüfen**")
        ...
        if packet.get("evidence_assumption_rows"):
            st.markdown("**Evidenz- und Annahmengrenzen**")
            st.dataframe(
        ...
        quality_checks = packet.get("briefing_quality_checks", [])
        if quality_checks:
            st.markdown("**Qualitätscheck der Ergebnislesart**")
            st.dataframe(
...
def build_learning_data_readiness_backlog(limit: int = 6) -> dict[str, Any]:
    ...
    operator_export_share_cards = build_data_readiness_registry_integration_operator_export_share_cards(
...
def render_learning_data_readiness_backlog():
    ...
    with st.expander("Konkrete nächste Plattform-Aktionen", expanded=True):
        ...
        integration_plan = backlog["integration_plan"]
        st.markdown(f"**{integration_plan['title']}**")
        st.caption(integration_plan["plain_language_note"])
        plan_rows = [
            {
                "Parameter": row["label"],
                "Status": row["status"],
                "Benötigte Inputs": " · ".join(row["required_inputs"][:2]),
                "Dateien": " · ".join(row["proposed_files"][:3]),
                "Tests": " · ".join(row["test_plan"][:2]),
                "Workflow": row["workflow_api"],
                "Guardrail": row["guardrail"],
            }
            for row in integration_plan["plans"]
        ]
        if plan_rows:
        ...
        st.markdown(f"**{progress_timeline['title']}**")
        st.caption(progress_timeline["plain_language_note"])
        progress_rows = [
            {
                "Rang": phase["rank"],
                "Phase": phase["phase"],
                "Status": phase["status"],
                "Öffnen": phase["what_to_open"],
                "Was sehe ich?": phase["plain_language_result"],
                "Guardrail": phase["guardrail"],
            }
            for phase in progress_timeline["phases"]
        ]
        ...
        export_bundle = backlog["registry_integration_operator_export_bundle"]
        st.markdown(f"**{export_bundle['title']}**")
        st.caption(export_bundle["plain_language_note"])
        st.dataframe(pd.DataFrame([{
            "Copy-safe": "ja" if export_bundle["copy_safe"] else "nein",
            "SHA256": export_bundle["packet_sha256"],
            "GET-Routen": export_bundle["safe_route_count"],
            "Stop-Gate": export_bundle["stop_condition"],
        }]), use_container_width=True, hide_index=True)
        ...
        st.markdown(f"**{export_review_stoplight['title']}**")
        st.caption(export_review_stoplight["plain_language_note"])
        st.dataframe(pd.DataFrame([
            {
                "Status": export_review_stoplight["overall_status"],
                "Darf Status-Handoff teilen?": "ja" if export_review_stoplight["may_share_status_handoff"] else "nein",
                "Erste Aktion": export_review_stoplight["first_safe_action"],
                "Stop-Gate": export_review_stoplight["stop_condition"],
            }
        ]), use_container_width=True, hide_index=True)
    ...
    with st.expander("Konkrete Snapshot-Requests (noch kein Import)", expanded=False):
        requests = backlog["connector_snapshot_requests"]
        if requests:
            for request in requests:
                st.markdown(f"**{request['parameter_label']}** — {request['source_label']} Tabelle `{request['table_code']}`")
                st.caption(
                    f"Endpoint: {request['endpoint_url']} · Datei: {request['suggested_filename']} · "
                    f"Nächster sicherer Schritt: {request['next_safe_action']} Guardrail: {request['guardrail']}"
                )
        else:
